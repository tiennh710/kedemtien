...
export async function hmac(value: string, purpose: 'otp' | 'session' = 'session') {
  const secret = purpose === 'otp' ? env.OTP_HMAC_SECRET : env.SESSION_SECRET;
  if (!secret || secret.length < 24) throw new Error(`Secret ${purpose} chưa được cấu hình an toàn.`);
...
export function isAdminEmail(email: string) {
  const allowed = (env.ADMIN_EMAILS ?? '').split(',').map(normalizeEmail).filter(Boolean);
...
export async function getSessionUser(request: Request): Promise<SessionUser | null> {
  ...
  `).bind(tokenHash, Date.now()).first<{ id: number; email: string }>();
  if (!row) return null;
  void getD1().prepare('UPDATE sessions SET last_seen_at = ? WHERE token_hash = ?').bind(Date.now(), tokenHash).run();
...
export function safeReturnTo(value: unknown) {
  return typeof value === 'string' && value.startsWith('/') && !value.startsWith('//') ? value : '/tai-khoan';
}
